    ...
    def run_train(self, args):
        ...
        for episode in range(args.episode_num):
            ...
            for step in range(args.episode_size):
                ...
                def step_game(game):
                    idx = game.get_id()
                    # rarenum = [1,2,3,4,5,6,7,8]
                    lastrarenum[action[idx][0].item()]+=1
                    normalized_state, step_reward, finished = game.step_normalized(action[idx][0], rarenum=rarenum)
                    state.screen[idx, :] = torch.from_numpy(normalized_state.screen)
                    state.variables[idx, :] = torch.from_numpy(normalized_state.variables)
        ...
        if "oblige" in args.vizdoom_config:
            ...
            for i, [wad_file, map_id] in enumerate(game_levels):
                ...
                while True:
                    ...
                    step_state, _, finished = game.step_normalized(action[0][0], rarenum=5000)
            ...
            while True:
                ...
                # compute an action
                action = policy.get_action(state, action, action_dist=False)
                step_state, _, finished = game.step_normalized(action[0][0], rarenum=np.ones(8)*500)
                policy.set_non_terminal(torch.zeros(1, 1) if finished else torch.ones(1, 1))
                if finished:
                    ret = game.get_episode_return()
                    print("episode return: {}".format(ret))
